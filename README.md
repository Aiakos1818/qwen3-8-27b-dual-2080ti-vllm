...
后台运行用 `--start`（pidfile/日志在 `~/.cache/kv-offload-panel/panel-<port>.{pid,log}`），
`--status` 查存活、`--stop` 停，状态按服务端口区分，可多个面板并存。
...
- **MTP 的 n 越大解码越快**（2026-09-18 更正本行旧结论）：接受率虽随 n 下降，但一轮里 draft 只跑
  ...
  31.5K/215K/250K/449K 四处再快 +4.8~13%**。**所有 profile 默认 n=6**（500K 档零代价：池 509,877
  ...
  （而非每 token 吞吐）判的。
- **lm_head 量化（改 checkpoint）：int8 甜点，净吞吐 +17~21%**（2026-09-19 实测）：head 从
  bf16 2.37 GiB 量化到 int8 1.30 GiB（走 Humming kernel，相对误差 0.517%），每步耗时
  31.5K 52.1→46.3 ms、215K 71.0→65.2 ms、449K 96.4→87.8 ms，MTP 接受率与基线持平 →
