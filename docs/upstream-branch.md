...
python3 scripts/tools/monitor_kv_offload_web.py --start            # 后台（--stop 结束）
python3 scripts/tools/monitor_kv_offload_web.py --status           # pid / 存活 / /healthz
python3 scripts/tools/monitor_kv_offload_web.py --stop             # 停 --port 上的面板
...
- **`--stop` 不靠模式匹配**：读 pidfile → SIGTERM → 最多等 10 s → 删 pidfile；发信号前用
  ...
  （与 `stop_server.sh` 同思路）。退出码：`0` 已停 / `1` 找到但异常（含校验不通过、超时未退出）/
  `2` 无实例可停。
- **`--status`**：打印 pid 与存活判断，再探测 `http://<host>:<port>/healthz`；未运行返回 `1`。
