...
| 组件 | 本分支路线（`sm75-upstream`） | 基础路线 | 用途 | 上游 / 引用 |
| --- | --- | --- | --- | --- |
| vLLM | `main` @ `0.26.1rc1.dev2278+g49f68ba24`（vLLM 侧分支 `sm75-upstream` @ `059727bfa`） | `v0.27.1` / `6e448d0ea9bf3d88d898b65449ca6dc2aec170ac` | API、TP、KV Cache、MTP、CUDA Graph | https://github.com/vllm-project/vllm |
...
- vLLM `sm75-upstream` @ `059727bfa`：让 SM75 的投机验证留在 FlashInfer native decode 路径，MTP 下保住 FULL cudagraph（单并发稳态解码 54.1 → 37.5 ms/步，见 [upstream-branch.md](upstream-branch.md) §6）。
