...
LOG="$T/runner_v1auto.log"
: >"$LOG"

echo "[v1] launching VLLM_USE_V2_MODEL_RUNNER=0 (see run_256k_mtp6_v1auto.sh:83)"
setsid bash "$T/run_256k_mtp6_v1auto.sh" --head8bit >"$LOG" 2>&1 &
...
if "$TOOLS/wait_server.sh" "$LOG" 900 8000; then
  ...
  "$VLLM_PYTHON" "$REPO/benchmarks/run_context_ttft.py" \
    ...
    --output $T/ab_runner_v1auto.json 2>&1 | tee $T/bench_v1auto.out | tail -12
else
