...
H_KV, D, PAGE, KV_LEN = 2, 256, 16, 250000
...
nbytes = npages * 2 * PAGE * H_KV * D

for threads in (128, 256, 512):
    for nchunks in (68, 136, 272, 544):
        mod.probe(kv, idx, KV_LEN, H_KV, D, PAGE, nchunks, threads)
        ...
        print(f"  threads={threads:4d} chunks={nchunks:4d}  {ms:7.3f} ms  {nbytes/ms/1e6:7.1f} GB/s")
