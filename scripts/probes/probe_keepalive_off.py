#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Disabled-mode regression: VLLM_PIN_MIN_TOKENS=0 -> A must NOT be protected."""
import os
import random
import time
...
def send(ids, note):
    ...
    print(f"[{note}] prompt={u.prompt_tokens} cached={cached} wall={time.time()-t0:.1f}s", flush=True)
    return cached
...
send(make_ids(25000, seed=2), "B(25k)")
send(A, "A replay AFTER B (disabled pin -> expect 0)")
