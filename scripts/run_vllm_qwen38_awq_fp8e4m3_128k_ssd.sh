...
# (CPU staging primary + disk/fs secondary) — test profile.
#
...
#
# Paths / model come from .env (copy config/vllm-128k-ssd.env.example).
set -Eeuo pipefail
...
# chunk per block). 128K -> 82 chunks -> 4.58e9. Below that, an evicted
# full-length session silently restores nothing. Costs /dev/shm and RAM in
...
# never fill the filesystem (0 = no cap, i.e. upstream behavior). 64 GiB is
# ~14 full-length 128K sessions.
