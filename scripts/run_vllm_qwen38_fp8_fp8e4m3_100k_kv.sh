#!/usr/bin/env bash
# Qwen3.8-27B FP8 (block-wise dynamic e4m3 weights) + fp8_e4m3 KV, 100K pool.
#
# Weight-quantization variant of run_vllm_qwen38_awq_fp8e4m3_100k.sh: identical
# KV-optimization feature set (keep-alive pin, Mamba/GDN durable anchors, session
# park/resume to RAM and optional SSD), but loads the FP8-weight checkpoint with
# `--quantization fp8` instead of AWQ-INT4 (compressed-tensors).
#
# The KV-optimization code is weight-quantization agnostic: it moves raw KV
# blocks. Only the non-KV memory baseline changes, so the KV pool budget
# (KV_CACHE_MEMORY_BYTES) may need re-calibration for the larger FP8 weights.
# See reports/2026-09-fp8-kv/ and docs/kv-optimization/.
#
# Paths / model come from .env (copy config/vllm-fp8-100k.env.example).
set -Eeuo pipefail
...
# --- weight quantization ----------------------------------------------------
: "${QUANTIZATION:=fp8}"                      # FP8 weights (block-wise e4m3)
...
: "${KV_ENGINE_ID:=qwen38-27b-fp8-100k}"
...
# FP8 weights trigger norm/act quantization fusions that JIT-compile via Triton,
# which shells out to `ninja`; it lives in the vLLM venv's bin, so prepend it.
...
if [ -n "${VLLM_SSD_ROOT:-}" ]; then
  ...
  export VLLM_SSD_ONLY="${VLLM_SSD_ONLY:-0}"
fi
...
ARGS=(
  ...
  --max-num-batched-tokens "$MAX_NUM_BATCHED_TOKENS" --enable-chunked-prefill
  --no-async-scheduling
  --skip-mm-profiling
